            ...
            <form action="{{ route('admin.product-category.update', $productCategory->id)}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card card-dark">
                    <div class="card-header">
                        ...
                        <a href="{{ route('admin.product-category.index')}}" class="btn btn-outline-warning float-right btn-sm">{{ __('View All')}}</a>
                    </div>
                    <div class="card-body">
                        <div class="form-group">
                            <label for="name">Name</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ $productCategory->name}}">
                        </div>

                        @if($productCategory->thumbnail)
                            <div class="form-group">
                                <img src="{{ asset($productCategory->thumbnail)}}" alt="{{ $productCategory->name}}">
                            </div>
                        @endif
